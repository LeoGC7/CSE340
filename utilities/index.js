...
Util.getNav = async function (req, res, next) {
  ...
  let list = "<ul class='nav-items-container'>"
  list += '<li class="nav-item"><a href="/" title="Home page" class="nav-item">Home</a></li>'
...
Util.buildClassificationGrid = async function(data){
  ...
  if(data.length > 0){
    ...
    data.forEach(vehicle => { 
      grid += '<li>'
      grid +=  '<a class="classification-image-container" href="../../inv/detail/'+ vehicle.inv_id 
...
Util.buildClassificationList = async function (classification_id = null) {
  ...
  let classificationList =
    '<select name="classification_id" id="classificationList" class="form-input select-input" required>'
  classificationList += "<option value='' selected='true' disabled>Choose a Classification</option>"
  data.rows.forEach((row) => {
